...
app.post('/control-envios/finalizar', async (req, res) => {
  ...
  try {
    ...
    for (const item of productos) {
      ...
      if (item.tipo === 'agregado') {
        ...
        if (productoPedido.length === 0) {
          ...
          // Si ya existía el agregado, actualizar su precio si viene
        ...
        // 🟢 NUEVO: Actualizar precio unitario en el detalle del pedido si viene editado
        if (item.precio_unitario !== undefined && item.precio_unitario !== null) {
          await conn.query(`
            UPDATE pedido_detalle
            SET precio_unitario = ?
    ...
    // 🔥 Recalcula total usando los precios recién actualizados en pedido_detalle
    await conn.query(`
      UPDATE pedidos p
      SET total = (
        SELECT COALESCE(
    ...
    const [movimientos] = await db.query(
      `(SELECT 
       ...
       (SELECT 
          ...
          CAST(CONCAT('Préstamo Recibido - ', pr.prestamista) AS CHAR(255)) COLLATE utf8mb4_unicode_ci AS concepto,
          CAST(pr.monto_original AS DECIMAL(10,2)) AS monto,
          CAST(pr.cuenta_destino AS CHAR(50)) COLLATE utf8mb4_unicode_ci AS forma_pago,
          pr.fecha_registro AS fecha
        FROM prestamos pr
        WHERE pr.fecha_registro >= ?)
